...
object Day9 extends App {
  val in = scala.io.Source.fromFile("src/aoc2017/day9.input.txt").mkString.toList

  @scala.annotation.tailrec
  def solution(s: List[Char],
               score: Int,
               garbage: Int,
               depth: Int,
               inGarbage: Boolean): (Int, Int) = s match {
  ...
  val (score, garbageCount) = solution(in, 0, 0, 0, inGarbage = false)
